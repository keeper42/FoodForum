...
<head>
    ...
    <script type="text/javascript">
        ...
        document.cookie = 'now_path' + "="+ '../personal_page/personal_page.php' + ";path=/";
                ...
                <div class="col-md-10 column">
                    ...
                    <div class="row clearfix" >
                        ...
                        <div class="col-md-4 column">
                            <a  data-toggle="modal" href="#modify_userinfo"><img width="90%" alt="<?php echo $picture;?>" src="<?php echo $picture;?>" class="img-circle" style="margin: 5%;" onload="rectangle(this)"/></a>
                    ...
                    <div class="tabbable" id="tabs-652527">
                        ...
                        <div class="tab-content">
                           ...
                           <div class="tab-pane" id="panel-message"><br>
                            <?php 
                                    $myMessageAll = new my_search('*','message',"user2_ID = $user || user1_ID = $user ORDER BY time DESC");
                                    //print_r($myMessageAll);echo '<br>';
                                    $box_style = array( 
                                        0 =>"success",
                                        1 => "info",
                                        2 => "danger",
                                        3 => "warning");
                                    for($i=0;$i<$myMessageAll->num;$i++){
                                        //echo $myMessageAll->rows[$i]['user2_ID']."++++++++++++++++++".$user."+<br>";
                                        $a = $myMessageAll->rows[$i]['user2_ID'];
                                        $b = $user;
                                         if($myMessageAll->rows[$i]['user2_ID']!=$user){
                                            $myMessage = new my_search('*','user'," ID = ".$myMessageAll->rows[$i]['user2_ID'] );
                                           // print_r("++++++++++++++++++++++++++++");
                                         }   
                                        else $myMessage = new my_search('*','user',"ID = ".$myMessageAll->rows[$i]['user1_ID'] );
                                        //print_r($myMessage);

                                        if($myMessage->rows[0]['picture']=='default')
                                            $picture = "../image/user_image/default";
                                        else $picture = "../image/user_image/".$myMessage->rows[0]['ID'].$myMessage->rows[0]['picture'] ;
                                        if($myMessageAll->rows[0]['user2_ID']!=$user)$word = "向".$myMessage->rows[0]['name']."发送消息";
                                        else $word = "来自".$myMessage->rows[0]['name']."的消息";
                              ...
                              <div class="alert alert-<?php echo $box_style[$i%4]; ?> alert-dismissable" style="position: relative;">
                                 <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                    <div style="display: inline;">
                                        <a href="../personal_page/visit_page.php" onclick="set_target(<?php echo $myMessage->rows[0]['ID']?>)">
                                    ...
                                    <div style="display:block; position: absolute;left: 23%;top: 7%; width: 75%">
                                         <h4 style="">
                                            ...
                                            <hr>
                                            消息内容：<?php  echo $myMessageAll->rows[$i]['content'] ?>
                                    </div>   
                                    <div style="display:block;position: absolute;left: 23%;bottom:  7%;width: 75%">
                                        <hr>
                                        时间：<?php  echo $myMessageAll->rows[$i]['time'] ?>
                                    </div>  
                                    <div style="display: inline;position: absolute;right:2%;bottom: 5%;">
                                        <button  class="btn btn-warning" role="button"  data-toggle="modal" data-target="#sent_message"   onclick="set_target(<?php echo $myMessage->rows[0]['ID']; ?>);set_name('<?php echo $myMessage->rows[0]['name']; ?>')">回复</button>
                                        &nbsp;

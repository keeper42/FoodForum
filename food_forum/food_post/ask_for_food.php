...
<head>
    ...
    <script type="text/javascript">
        ...
        document.cookie = 'now_path' + "="+ '../food_post/ask_for_food.php' + ";path=/";
    </script>
</head>
<body>
   <?php include '../overlook_food/register.php' ;?>
    ...
    <div class="container" style="position: relative;">
        <br><h2>欢迎来到打饭吧</h2>
        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#ask_food_post_Modal" style="position: absolute;right: 2%;bottom: 40px;" >发打饭贴</button>
        <hr>
        <?php include '../other/ask_food_post.php'; ?>
    </div>
    <main id="mainMenu" class="container">
    <?php include '../other/sent_message.php'; ?>
    <div class="tab-pane" id="panel-message">
    <?php 
            $myFoodPost = new my_search('*','ask_for_food t,user u'," t.user_ID = u.ID ");
            //print_r($myFoodPost);
            $box_style = array( 
                0 =>"success",
                1 => "info",
         ...
         <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            <div style="display: inline;">
                <a href="../personal_page/visit_page.php" onclick="set_target(<?php echo $myFoodPost->rows[$i]['ID']?>)">   
                            <img class="img-rounded" onload="rectangle(this)" id="" style="width: 20%; display:inline; " src="<?php echo $picture; ?>" alt="<?php echo $picture; ?>"
                             ">
                     </a>
            </div>
            <div style="display:block; position: absolute;left: 23%;top: 7%; width: 75%">
                 <h4 style="">
                        来自<strong><?php  echo $myFoodPost->rows[$i]['name'] ?></strong>打饭求助
                    </h4> 
                    <hr>
                    食客ID：<?php  echo $myFoodPost->rows[$i]['ID'] ?>&emsp;
                    食客等级：<?php  echo $myFoodPost->rows[$i]['level'] ?>&emsp;
                    打饭积分：<?php  echo $myFoodPost->rows[$i]['credit'] ?><br>
                    消息内容：<?php  echo $myFoodPost->rows[$i]['note'] ?><br>
                    送去地点：<?php  echo $myFoodPost->rows[$i]['address'] ?><br>
                    送饭时间：<?php  echo $myFoodPost->rows[$i]['start_time'] ?>到<?php  echo $myFoodPost->rows[$i]['start_time'] ?>之间<br>
            </div>   
            <div style="display:block;position: absolute;left: 23%;bottom:  7%;width: 75%">
                <hr>
                时间：<?php  echo $myFoodPost->rows[$i]['time'] ?>
            </div>  
            <div style="display: inline;position: absolute;right:2%;bottom: 5%;">
                <button type="button" class="btn btn-warning"  data-toggle="modal" data-target="#sent_message" onclick="set_target(<?php echo $myFoodPost->rows[$i]['user_ID']; ?>);set_name('<?php echo $myFoodPost->rows[$i]['name']; ?>')">帮他一把</button>
                &nbsp;
                <button type="button" data-dismiss="alert" aria-hidden="true" class="btn btn-warning">爱莫能助</button>
            </div>  
